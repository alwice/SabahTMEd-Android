...
public class disease_herb_list extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        temp = intent.getExtras().getString("DiseaseName");
        ...
        try {
            Cursor cursor = dbHelper.QueryData("select herb_name from herbs where herb_disease='" + temp + "'");
            if (cursor != null) {
                if (cursor.moveToFirst()) {
                    do {
                        Item item = new Item();
                        item.setDiseaseListHerb(cursor.getString(0));
        ...
        adapter = new Adapter(this, R.layout.herb_list, arrayListDiseaseHerb);
        ...
        listDiseaseHerb.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                passHerbName = diseaseListHerb.getDiseaseListHerb().toString();

                adapter.getItemId(position);
                int type =2;

                Intent intentDisHerb = new Intent(disease_herb_list.this, disease_herb_information.class);
                intentDisHerb.putExtra("HerbName", passHerbName);
                intentDisHerb.putExtra("DiseaseName",temp);
                intentDisHerb.putExtra("Mode",type);
